...
export function ExpensesList({ transactions }) {
  ...
  const getShortName = (description) => {
              ...
              >
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs text-muted-foreground capitalize">• {t(expense.account)}</span>
                  </div>
                </div>
                <span className="font-semibold text-red-500">{formatCurrency(Math.abs(expense.amount))}</span>
              </div>
            ))}
          </div>
        )}
