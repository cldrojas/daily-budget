...
import type { Account, Budget, Transaction, TransactionType } from '@/types'

const debug = (message: string, value?: any) => console.log(`${message}: ${value}`)
...
export function useBudget() {
  ...
  debug('[render] autoSave', autoSave)
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (savedData) {
        ...
        setAutoSave(parsedData.autoSave || true)
      }
    }
  }, [])
  ...
  useEffect(() => {
    ...
    if ((!lastCheckedDay || !isSameDay(today, lastCheckedDay))) {
      debug('first check or newDay')
      // If there was a previous day, move remaining amount to savings if config enabled
      if (lastCheckedDay && remainingToday > 0 && autoSave) {
        debug('prev day with remaining and autosave')
  ...
  }, [isSetup, lastCheckedDay, faketrigger])
